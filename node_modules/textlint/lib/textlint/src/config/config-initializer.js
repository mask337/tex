...
var getTextlintDependencyNames = function (dir) {
    return readPkg(dir)
        ...
        return pkgNames.filter(function (pkgName) {
            var ruleOrFilter = pkgName.indexOf(package_prefix_1.PackageNamePrefix.filterRule) !== -1 ||
                pkgName.indexOf(package_prefix_1.PackageNamePrefix.rule) !== -1;
...
exports.createConfigFile = function (options) {
    ...
    return getTextlintDependencyNames(dir).then(function (pkgNames) {
        ...
        var filters = pkgNames
            ...
            return pkgName.indexOf(package_prefix_1.PackageNamePrefix.filterRule) !== -1;
        })
            .map(function (filterName) {
            return filterName.replace(package_prefix_1.PackageNamePrefix.filterRule, "");
        });
        var rules = pkgNames
            .filter(function (pkgName) {
            return pkgName.indexOf(package_prefix_1.PackageNamePrefix.rule) !== -1;
        })
            .map(function (filterName) {
            return filterName.replace(package_prefix_1.PackageNamePrefix.rule, "");
        });
